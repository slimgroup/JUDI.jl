...
# Source wavelet
f0 = 0.01f0     # MHz
wavelet = Array{Array}(undef, nsrc)
for j=1:nsrc
    wavelet[j] = ricker_wavelet(time, dt, f0)
end
...
# Forward
d_obs = extended_source_modeling(model, wavelet, recGeometry, nothing, weights, nothing, 1:1, 'F', 1, opt)

# Adjoint
w_hat = extended_source_modeling(model, wavelet, recGeometry, d_obs.data, nothing, nothing, 1:1, 'F', -1, opt)

# Born
d_lin = extended_source_modeling(model, wavelet, recGeometry, nothing, weights, dm, 1:1, 'J', 1, opt)

g = extended_source_modeling(model, wavelet, recGeometry, d_obs.data, weights, nothing, 1:1, 'J', -1, opt)
