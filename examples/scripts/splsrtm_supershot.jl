# Example for 2D sparsity-promoting LS-RTM w/ computational simultaneous sources
# The implementation of sim sources follows the journal article
...
# Geophysics, 2012"
# Super-shot experiments are generated by superposition of sources and shot records
# with weights drawn from normal distribution
...
for j = 1:niter
    ...
    xsrc = [[q.geometry.xloc[i][1] for i = 1:nsrc] for j = 1:batchsize]
    ysrc = [[0.0f0] for j = 1:batchsize]
    zsrc = [[q.geometry.zloc[i][1] for i = 1:nsrc] for j = 1:batchsize]
    ...
    # Create wavelet
    wavelet = Array{Array{Float32,2}}(undef, batchsize)
    for k = 1:batchsize
        wavelet[k] = zeros(Float32, src_geometry.nt[1], nsrc)
        wavelet[k][:,1:nsrc] = q.data[1]*weights[k:k,:]
    end

    q_j = judiVector(src_geometry, wavelet)    # super shot for current iteration

    data_j = Array{Array{Float32,2}}(undef, batchsize)
    for k = 1:batchsize
        data_j[k] = sum(weights[k,:].*dobs.data)
    end

    dobs_j = judiVector(Geometry(dobs.geometry.xloc[1],dobs.geometry.yloc[1],dobs.geometry.zloc[1]; dt=dobs.geometry.dt[1], t=dobs.geometry.t[1], nsrc=batchsize),data_j)

    println("LS-RTM Iteration: $(j) w/ computational sim sources")
    phi, g = lsrtm_objective(model0, q_j, dobs_j, Mr*x; nlind=true,options=opt)
    g = C*Mr'*vec(g)
    ...
    global tau .= t[j]
    global tau[findall(flag)] = deepcopy(tau1[findall(flag)])
