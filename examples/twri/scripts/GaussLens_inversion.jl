...
function funWRI!(F, G, x)
    ...
    fun, g, gy = twri_objective(model0, fsrc, dat, nothing; optionswri=optwri)
    F = fun
    isnothing(G) && return fun
    #isnothing(g_const) && (global g_const = .1/maximum(abs.(g)))
    G .= g_const .* g[:]
...
## Run FWI
#g_const = nothing
x0_fwi = zeros(R, length(model0.m))
...
## Run WRI
#g_const = nothing
x0_wri = zeros(R, length(model0.m))
