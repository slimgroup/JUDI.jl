...
export SrcRecReciprocal, transducer
...
"""
    SrcRecReciprocal(d_obs, q)

The data d_obs is assumed to be generated by sequential fired sources q,
SrcRecReciprocal outputs the data and source as if we switch the source/receiver locations thanks to reciprocity

SrcRecReciprocal outputs the data and source as if we switch the source/receiver locations thanks to reciprocity

Parameters:
* `d_obs`: data (judiVector)
* `q`: source (judiVector) which generates the aformentioned data
"""
function SrcRecReciprocal(d_obs::judiVector, q::judiVector)
    ...
    reci_data = Array{Array{Float32,2}}(undef, nsrc)    # reciprocal data
    for i = 1:nsrc 
        reci_data[i] = zeros(Float32,d_obs.geometry.nt[1],nrec)
        for j = 1:nrec 
            reci_data[i][:,j] = d_obs.data[j][:,i]
        end
    end
