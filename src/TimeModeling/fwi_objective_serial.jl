...
# Setup time-domain linear or nonlinear foward and adjoint modeling and interface to OPESCI/devito
    ...
    elseif ~isempty(options.frequencies)
        ...
        dPredicted, ph, pv = pycall(ac.forward_modeling, PyObject, modelPy, PyReverseDims(copy(transpose(src_coords))), PyReverseDims(copy(transpose(qIn))), PyReverseDims(copy(transpose(rec_coords))), save=true, free_surface=options.free_surface)
    	# Data misfit
        if isempty(options.gs)
            # wrong misfit for trace based
            argout1 = misfit(dPredicted, dObserved, options.normalize)
        ...
        argout2 = pycall(ac.adjoint_born, Array{Float32}, modelPy, PyReverseDims(copy(transpose(rec_coords))), PyReverseDims(copy(transpose(residual))),
                         ph=ph, pv=pv, is_residual=true, free_surface=options.free_surface)
    end
    argout2 = remove_padding(argout2, model.nb, true_adjoint=options.sum_padding)
    if options.limit_m==true
        argout2 = extend_gradient(model_full,model,argout2)
