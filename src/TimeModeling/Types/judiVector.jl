...
export write_shot_record, get_data, convert_to_array, rebuild_jv, merge
...
Merge multiple-source judiVector `x` with `weights` (default is all ones). \\
...
(1) Merge all the shot records in judiVector `dobs`

    dobs1 = merge(dobs)

(2) Merge all the shot records in `dobs` with `weights`

    dobs1 = merge(dobs, weights) # `weights` is a `AbstractVector`

(3) Generate multiple shot records, each of which is generated by merging `dobs` with different weights

    dobs1 = merge(dobs, weights) # `weights` is a `AbstractMatrix` and each row is used for a different merge operation

"""

# merge(judiVector, AbstractVector)
function merge(v::judiVector{T, Array{T,N}}, w::AbstractVector{T}) where {T, N}
    ...
    for i = 1:v.nsrc
        dict[i] = Dict{Tuple{T,T,T}, Vector{T}}()
        for j = 1:length(v.geometry.xloc[i])
            xloc = v.geometry.xloc[i][j]
            if length(v.geometry.yloc[i]) == 1
                yloc = v.geometry.yloc[i][1]
            else
                yloc = v.geometry.yloc[i][j]
            end
            zloc = v.geometry.zloc[i][j]
            dict[i][(xloc, yloc, zloc)] = w[i] .* v.data[i][:,j]
        end
    end
...
# merge(judiVector, Matrix)
function merge(v::judiVector{T, Array{T,N}}, w::AbstractMatrix{T}) where {T, N}

    vec_merge = merge(v, view(w, 1, :))
    for i = 2:size(w,1)
        push!(vec_merge, merge(v, view(w, i, :)))
    end
    return vec_merge
...
# merge(judiVector, Matrix)
function merge(v::judiVector{T, Array{T,N}}) where {T, N}

    return merge(v, ones(T, v.nsrc))
