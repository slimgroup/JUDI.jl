...
def forward_rec(model, src_coords, wavelet, rec_coords, space_order=8, 
    free_surface=False, dt_comp=None):
    """
    Forward modeling of a point source.
    Outputs the shot record.
    """
    rec, _ = forward(model, src_coords, rec_coords, wavelet, save=False, dt_comp=dt_comp,
                     space_order=space_order, free_surface=free_surface)
    return rec.data

def forward_rec_wf(model, src_coords, wavelet, rec_coords,
                   space_order=8, free_surface=False, dt_comp=None):
    """
    Forward modeling of a point source.
    Outputs the shot record.
    """
    rec, u = forward(model, src_coords, rec_coords, wavelet, save=True, dt_comp=dt_comp,
                     space_order=space_order, free_surface=free_surface)
    return rec.data, u


def forward_no_rec(model, src_coords, wav, space_order=8, free_surface=False, dt_comp=None):
    """
    ...
    _, u = forward(model, src_coords, None, wav, space_order=space_order,
                   save=True, free_surface=free_surface, dt_comp=dt_comp)
    return u.data


def forward_wf_src(model, u, rec_coords, space_order=8, free_surface=False, dt_comp=None):
    """
    ...
    rec, _ = forward(model, None, rec_coords, None, space_order=space_order,
                     free_surface=free_surface, q=wf_src, dt_comp=dt_comp)
    return rec.data


def forward_wf_src_norec(model, u, space_order=8, free_surface=False, dt_comp=None):
    """
    ...
    if isinstance(u, TimeFunction):
        ...
        wf_src.data[:] = u[:]
    _, u = forward(model, None, None, None, space_order=space_order, dt_comp=dt_comp,
...
def adjoint_rec(model, src_coords, rec_coords, data,
    ...
    """
    rec, v = adjoint(model, data, src_coords, rec_coords, dt_comp=dt_comp,
                     space_order=space_order, free_surface=free_surface)
    return rec.data
...
def adjoint_wf_src(model, u, src_coords, space_order=8, free_surface=False, dt_comp=None):
    """
...
def adjoint_wf_src_norec(model, u, src_coords, space_order=8, 
    free_surface=False, dt_comp=None):
    """
    Adjoint/backward modeling of a full wavefield (full wavefield as adjoint source).
    Outputs the full adjoint wavefield.
    ...
    if isinstance(u, TimeFunction):
        ...
        wf_src.data[:] = u[:]
    _, v = adjoint(model, None, None, None, space_order=space_order, dt_comp=dt_comp,
                     save=True, free_surface=free_surface, q=wf_src)
    return v.data


# Gradient only
def grad_fwi(model, recin, rec_coords, u, space_order=8, free_surface=False, dt_comp=None):
    g = gradient(model, recin, rec_coords, u, space_order=space_order, dt_comp=dt_comp,
...
def born_rec(model, src_coords, wavelet, rec_coords,
    ...
    """
    rec, _ = born(model, src_coords, rec_coords, wavelet, save=False, dt_comp=dt_comp,
                  space_order=space_order, free_surface=free_surface)
    return rec.data
...
def J_adjoint(model, src_coords, wavelet, rec_coords, recin, space_order=8, checkpointing=False, 
    free_surface=False, n_checkpoints=None, maxmem=None, freq_list=[], dft_sub=None, dt_comp=None):
    """
    ...
    * Standard zero lag cross correlation over time
    ...
    if checkpointing:
        grad = J_adjoint_checkpointing(model, src_coords, wavelet, rec_coords,
                                       ...
                                       maxmem=maxmem, dt_comp=dt_comp)
    elif len(freq_list) > 0:
        grad = J_adjoint_freq(model, src_coords, wavelet, rec_coords, recin, dt_comp=dt_comp,
        ...
        grad = J_adjoint_standard(model, src_coords, wavelet, rec_coords, recin,
                                  is_residual=True, dt_comp=dt_comp, 
                                  space_order=space_order, free_surface=free_surface)
...
def J_adjoint_freq(model, src_coords, wavelet, rec_coords, recin, space_order=8, free_surface=False, 
    freq_list=[], is_residual=False, return_obj=False, dft_sub=None, dt_comp=None):
    """
    ...
    rec, u = forward(model, src_coords, rec_coords, wavelet, save=False,
                     space_order=space_order, free_surface=free_surface,
                     freq_list=freq_list, dft_sub=dft_sub, dt_comp=dt_comp)
...
def J_adjoint_standard(model, src_coords, wavelet, rec_coords, recin, space_order=8, 
    free_surface=False, is_residual=False, return_obj=False, dt_comp=None):
    """
    ...
    g = gradient(model, recin, rec_coords, u, space_order=space_order,
                 free_surface=free_surface, dt_comp=dt_comp)
    if return_obj:
        return .5*np.linalg.norm(recin)**2, g.data
    return g.data
...
def J_adjoint_checkpointing(model, src_coords, wavelet, rec_coords, recin, space_order=8, free_surface=False, 
    is_residual=False,n_checkpoints=None, maxmem=None, return_obj=False, dt_comp=None):
    """
    ...
    op, g = gradient(model, rec_coords, recin, space_order=space_order,
                     return_op=True, free_surface=free_surface, dt_comp=dt_comp)
